...
uint8_t HX711_ADC::update() 
{
	...
	uint8_t dout = HAL_GPIO_ReadPin(GPIOB,GPIO_PIN_1);
	if (!dout) 
	{
		conversion24bit();
		//if(s) Serial.print(s);
...
uint8_t HX711_ADC::conversion24bit()  //read 24 bit data and start the next conversion
{
	...
	conversionTime = DWT->CYCCNT - conversionStartTime;
	conversionStartTime = DWT->CYCCNT;
	...
	{ //read 24 bit data + set gain and start next conversion
		ms1Delay(); // could be required for faster mcu's, set value in config.h
